...
const UserDashboard = ({ user }) => {
  ...
  useEffect(() => {
    const obtenerMovimientosYRecargas = async () => {
      if (userState && userState.numeroCuenta) {
        try {
          // Obtener transacciones
          const responseTransacciones = await fetch(
            `http://localhost:3030/transacciones/${userState.numeroCuenta}`
          );
          const dataTransacciones = await responseTransacciones.json();

          // Obtener recargas
          const responseRecargas = await fetch(
            `http://localhost:3030/recargas/${userState.numeroCuenta}`
          );
          const dataRecargas = await responseRecargas.json();

          let saldo = parseFloat(userState.saldo); // Inicializamos el saldo con el saldo del usuario
          ...
          const movimientosConSaldo = dataTransacciones.map((transaccion) => {
            ...
            if (transaccion.cuentaOrigen === userState.numeroCuenta) {
              saldo -= monto;
              transaccion.tipo = 'Enviado';
              transaccion.color = 'gasto';
            } else if (transaccion.cuentaDestino === userState.numeroCuenta) {
              saldo += monto;
