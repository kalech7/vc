...
const UserDashboard = ({ user }) => {
  ...
  const [isBlocked, setIsBlocked] = useState(false); // Estado para verificar si alguna cuenta está bloqueada
  ...
  useEffect(() => {
    ...
    const fetchCuentas = async () => {
      try {
        ...
        const cuentas = Object.values(data);

        let hasBlockedAccount = cuentas.some(cuenta => cuenta.estado === "Bloqueada");
        ...
        setMenuVisible(!hasBlockedAccount);
        setCuentas(cuentas);
      } catch (error) {
  ...
  const openModalRecarga = () => {
    if (!isBlocked) { // Verificar si hay alguna cuenta bloqueada
      ...
      <div className={`content-container ${menuVisible ? 'menu-open' : ''}`}>
        {menuVisible && (
          <div className="sidebar">
            ...
            <h2>Menú</h2>
            <Link to="/dashboard" className="button">Mis productos</Link>
            <Link to="/transferencias" className="button">Transferencias</Link>
            <Link to="/movCuenta" className="button">Tus Movimientos</Link>
            <Link to="/perfil" className="button">Mi Perfil</Link>
            <Link to="/tickets" className="button">Soporte</Link>
          </div>
        )}
        <div className={`dashboard ${menuVisible ? 'menu-open' : ''}`}>
          {isLoading ? (
            <p>Cargando...</p>
          ) : userState ? (
            <>
                    ...
                    cuentas.map((cuenta, index) => (
                      <div key={index} className="cuenta-item">
                        <div className="info-box">
                          ...
                          <div className="info-value">{cuenta.numeroCuenta}</div>
                        </div>
              ...
              {!isBlocked && ( // Mostrar botones solo si no está bloqueado
                <>
